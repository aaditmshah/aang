...
abstract class OptionTrait {
	...
	public fold<A, B>(this: Option<A>, morphism: (value: A) => B, defaultValue: B): B {
	...
	public and<A, B>(this: Option<A>, that: Option<B>): Option<Pair<A, B>> {
		return this.isSome && that.isSome ? new Some(new Pair(this.value, that.value)) : None.instance;
	}
	...
	public flatMap<A, B>(this: Option<A>, arrow: (value: A) => Option<B>): Option<B> {
	...
	public flatMapUntil<A, B>(this: Option<A>, arrow: (value: A) => Option<Result<B, A>>): Option<B> {
		let result = this.flatMap(arrow).transposeOkay();
		while (result.isFail) result = arrow(result.value).transposeOkay();
		return result.value;
	}

	public filter<A, B extends A>(this: Option<A>, predicate: (value: A) => value is B): Option<B>;
	public filter<A>(this: Option<A>, predicate: (value: A) => boolean): Option<A>;
	public filter<A>(this: Option<A>, predicate: (value: A) => boolean): Option<A> {
	...
	public isSomeAnd<A>(this: Option<A>, predicate: (value: A) => boolean): this is Some<A>;
	public isSomeAnd<A>(this: Option<A>, predicate: (value: A) => boolean): this is Some<A> {
	...
	public isNoneOr<A>(this: Option<A>, predicate: (value: A) => boolean): boolean;
	public isNoneOr<A>(this: Option<A>, predicate: (value: A) => boolean): boolean {
	...
	public unzipWith<A, B, C>(
		...
		return this.isNone ? Pair.from(None.instance) : unzip(this.value).map(Some.of, Some.of);
	}

	public unzip<A, B>(this: Option<Pair<A, B>>): Pair<Option<A>, Option<B>> {
		return this.isNone ? Pair.from(None.instance) : this.value.map(Some.of, Some.of);
	}
	...
	public transposeMapOkay<A, B, E>(
		...
		return this.isNone ? new Okay(None.instance) : transpose(this.value).mapOkay(Some.of);
	}
	...
	public transposeMapFail<A, E, F>(
		...
		return this.isNone ? new Fail(None.instance) : transpose(this.value).mapFail(Some.of);
	}
	...
	public exchangeMapOkay<A, B, E>(
		...
		return this.isNone ? Task.okay(None.instance) : exchange(this.value).mapOkay(Some.of);
	}
	...
	public exchangeMapFail<A, E, F>(
		...
		return this.isNone ? Task.fail(None.instance) : exchange(this.value).mapFail(Some.of);
	}
	...
	public append<A extends Semigroup<A>>(this: Option<A>, that: Option<A>): Option<A> {
		if (this.isNone) return that;
		if (that.isNone) return this;
		return new Some(this.value.append(that.value));
	}

	public isSame<A extends Setoid<A>>(this: Option<A>, that: Option<A>): boolean {
		return this.isSome ? that.isSome && this.value.isSame(that.value) : that.isNone;
	}

	public isNotSame<A extends Setoid<A>>(this: Option<A>, that: Option<A>): boolean {
		return this.isSome ? that.isNone || this.value.isNotSame(that.value) : that.isSome;
	}

	public isLess<A extends PartialOrder<A>>(this: Option<A>, that: Option<A>): boolean {
		return that.isSome && (this.isNone || this.value.isLess(that.value));
	}

	public isNotLess<A extends PartialOrder<A>>(this: Option<A>, that: Option<A>): boolean {
		return that.isNone || (this.isSome && this.value.isNotLess(that.value));
	}

	public isMore<A extends PartialOrder<A>>(this: Option<A>, that: Option<A>): boolean {
		return this.isSome && (that.isNone || this.value.isMore(that.value));
	}

	public isNotMore<A extends PartialOrder<A>>(this: Option<A>, that: Option<A>): boolean {
		return this.isNone || (that.isSome && this.value.isNotMore(that.value));
	}

	public compare<A extends PartialOrder<A>>(this: Option<A>, that: Option<A>): Option<Ordering> {
		if (this.isNone) return new Some(that.isSome ? "<" : "=");
		if (that.isNone) return new Some(">");
		return this.value.compare(that.value);
	}

	public max<A extends TotalOrder<A>>(this: Option<A>, that: Option<A>): Option<A> {
		if (this.isNone) return that;
		if (that.isNone) return this;
		return new Some(this.value.max(that.value));
	}

	public min<A extends TotalOrder<A>>(this: Option<A>, that: Option<A>): Option<A> {
...
export class Some<out A> extends OptionTrait {
	...
	public static fromValid<A, B extends A>(value: A, validate: (value: A) => value is B): Option<B>;
	public static fromValid<A>(value: A, validate: (value: A) => boolean): Option<A>;
	public static fromValid<A>(value: A, validate: (value: A) => boolean): Option<A> {
